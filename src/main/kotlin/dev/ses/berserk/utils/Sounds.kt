...
enum class Sounds(private val sound8: String?, private val sound912: String?, private val sound13: String?) {
    ...
    private val version = Bukkit.getServer().javaClass.getPackage().name.split("\\.".toRegex())
        .dropLastWhile { it.isEmpty() }.toTypedArray()[3].replace("v", "").replace("R", "").replace("_", ".")
    ...
    private val sound: Sound?
        get() {
            ...
            return if (version.contains("1.8")) {
                (if (sound8 == null) Sound.valueOf("IRONGOLEM_DEATH") else Sound.valueOf(sound8)).also {
                    cachedSound = it
                }
            } else if (version.contains("1.9") || version.contains("1.10") || version.contains("1.11") || version.contains(
                    "1.12"
                )
            ) {
